package com.sp.app.state;

public class OrderState {
	// 주문상태 정보
	/*
		0:입금대기, 1:결제완료, 2:발송처리, 3:배송시작, 4:배송중, 5:배송완료,
		6:전체판매취소완료(관리자), 7:부분판매취소완료(관리자),
		8:주문자전체주문취소완료(배송전), 9:주문자부분주문취소완료(배송전)
		10:주문자전체반품취소완료(배송후), 11:주문자부분반품취소완료(배송후)
	*/
	public static final String[] ORDERSTATEINFO = 
		{"입금대기", "결제완료", "발송처리", "배송시작", "배송중", "배송완료", "전체판매취소", "부분판매취소",
				"전체주문취소", "부분주문취소", "전체반품취소", "부분반품취소"};

	// 주문상세상태 정보(관리자)
	/*
		0:구매확정대기, 1:구매확정완료, 2:자동구매확정,
		3:판매취소(관리자),
		4:주문취소요청(구매자), 5:주문취소완료(관리자), 
		6:교환요청(구매자), 7:교환접수(관리자), 8:교환발송완료(관리자), 9:교환불가(관리자),
		10:반품요청(구매자), 11:반품접수(관리자), 12:반품완료(관리자), 13:반품불가(관리자), 14:기타,
		15:환불요청(구매자), 16:부분반품요청(구매자), 17:부분환불요청(구매자),
		18:부분반품완료(구매자), 19: 부분환불완료(구매자), 20: 환불완료(관리자), 21: 환불접수(관리자), 22: 환불불가(관리자)
	*/
	public static final String[] DETAILSTATEMANAGER = 
		{"구매확정대기", "구매확정완료", "자동구매확정",
			"판매취소",
			"주문취소요청", "주문취소완료",
			"교환요청", "교환접수", "교환발송완료", "교환불가",
			"반품요청", "반품접수", "반품완료", "반품불가",
			"기타",
			"환불요청", "부분반품요청", "부분환불요청",
			"부분반품완료", "부분환불완료", "환불완료", "환불접수"
		};
	 
	// 주문상세상태 정보(구매자)
	public static final String[] DETAILSTATEINFO = 
		{"구매확정대기", "구매확정", "자동구매확정",
			"주문취소",
			"주문취소요청", "주문취소",
			"교환요청", "교환접수", "교환완료", "구매",
			"반품요청", "반품접수", "반품완료", "구매",
			"기타"};
}
