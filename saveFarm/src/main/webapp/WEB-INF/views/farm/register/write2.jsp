		...
		<div class="container">
		    <form name="registerForm" method="post" action="${pageContext.request.contextPath}/farm/register/${mode}" enctype="application/x-www-form-urlencoded">
		      ...
		      <div class="text-center">
		      	<c:if test="${ dto.state == 1 || mode == 'write' }">
		      		<button type="button" class="btn-primary" onclick="deleteOk();" style="background-color: red; color: black; border: none;">삭제하기</button>
		      	</c:if>
		        <button type="button" class="btn-primary" onclick="sendOk();">${mode=='update'?'수정완료':'신청완료'}</button>
		        <c:if test="${mode=='write'}">
		        <button type="button" class="btn-primary" onclick="location.href='${pageContext.request.contextPath}/farm/register/main'">신청취소</button>
		        </c:if>
       			<c:if test="${mode=='update'}">
		        	<button type="button" class="btn btn-primary" style="background-color: white; color: black; border: 1px solid #116530;" onclick="location.href='${pageContext.request.contextPath}/farm/register/detail?supplyNum=${dto.supplyNum}';">수정취소</button>
